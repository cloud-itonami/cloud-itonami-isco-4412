(ns mailrouting.actor
  "MailRoutingActor — the ISCO-08 4412 independent mail sorting &
  delivery actor as a `langgraph.graph/state-graph` (per
  ADR-2607011000 / CLAUDE.md Actors section). One graph run = one
  mail-handling operation request (intake → advise → govern → decide
  ...
  cloud-itonami-isco-4214's debtcollection.actor.
  ...
  The unconditional invariant: the Mail Advisor can never directly
  commit a record or dispatch a robot action the Mail Services
  Governor refuses — every commit-record! call is gated behind
  `:decide`."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [mailrouting.advisor :as advisor]
            [mailrouting.governor :as governor]
            [mailrouting.store :as store]))

(defn build-graph
  "Build a compiled MailRoutingActor graph. `store` implements
  `mailrouting.store/Store`. `advisor` implements
  `mailrouting.advisor/Advisor` (defaults to `mock-advisor`).
